...
- **나무는 수관 덩어리 없이 가지만으로 이뤄진다.** `TREE[].n` 은 줄기에서 바로 나오는
  ...
  **잎은 모든 가지에 단다**(속가지는 0.62배로 작게). 나무 하나가 가지 650~680개,
  ...
  설정의 `P.limb`(원가지 수 배율)을 내릴 것. 그다음이 `LEAF_PER`(참나무 5 · 전나무 14)다.
  **참나무 잎맥은 색으로 그린다** — 잎몸 위에 얇은 띠를 얹고 정점 색으로 잎몸(0x466032)과
  잎맥(0x859c45)을 갈랐다. 그래서 **참나무 잎 재질만 `vertexColors:true` 에 재질 색이
  흰색**이다(곱셈이 그대로 나오게). `mergeParts` 는 정점 색을 버리므로 참나무 잎은
  거치지 않는다. 잎 한 장이 삼각형 56장(잎몸 24 · 잎맥 32)이다.
  **참나무 잎은 한 장이 인스턴스 하나다**(`LEAF_PER.oak`=5) — 가지 끝 `LEAF_BACK`(0.1 m)
  앞부터 밑동 쪽으로 무작위 자리에 다섯 장을 흩어 단다(`putLeaf`). 그래서 **잎 인스턴스
