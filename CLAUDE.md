...
- **나무는 수관 덩어리 없이 가지만으로 이뤄진다.** `TREE[].n` 은 줄기에서 바로 나오는
  ...
  **잎은 모든 가지에 단다**(속가지는 0.62배로 작게). 나무 하나가 가지 650~680개,
  섬 전체가 삼각형 25.9M 이다 — 무거우면 `LIMB_SPACE` 를 올리거나(가지가 성겨진다)
  ...
  **참나무 잎은 판때기 하나가 인스턴스 하나다**(`LEAF_PER.oak`=3) — 가지 끝 `LEAF_BACK`(0.1 m)
  ...
  장수부터 줄일 것.
- **150 m 밖의 나무는 그리지 않는다**(`VIEW_DIST`). 인스턴스 메시는 통째로만 껐다 켤 수
  있으므로 **가지·잎을 `TILE`(128 m) 칸마다 따로 담고**(`TREE_TILES`) 칸 단위로 끈다
  (`updateTileVis`, 매 프레임). 나무 종류마다 메시 하나로 몰아 담으면 발밑의 나무 한 그루
  때문에 온 섬의 잎을 다 그리게 된다. 안개는 0.009 로 짙게 잡아 꺼지는 순간을 가린다.
