...
- **나무는 수관 덩어리 없이 가지만으로 이뤄진다.** `TREE[].n` 은 줄기에서 바로 나오는
  ...
  **곁가지 수는 그 가지의 길이에 비례한다** — `len/LIMB_SPACE`(0.8 m 마다 하나) 개를 내고
  소수부는 그 확률로 한 개 더 낸다. 깊이별 확률표는 없앴다. 곁가지가 부모의 0.2~0.58 로
  짧아지므로 **재귀가 길이로 저절로 멎는다** — `LIMB_DEPTH`(4)는 규칙이 아니라 안전장치다.
  **잎은 모든 가지에 단다**(속가지는 0.62배로 작게). 나무 하나가 가지 177~193개,
  섬 전체가 삼각형 25.9M 이다 — 무거우면 `LIMB_SPACE` 를 올리거나(가지가 성겨진다)
  설정의 `P.limb`(원가지 수 배율)을 내릴 것. 그다음이 `leafSpray` 의 장수(참나무 9 · 전나무 27)다. **참나무 잎은 한 장이 삼각형 48장**이다 —
